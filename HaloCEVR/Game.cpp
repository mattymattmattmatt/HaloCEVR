...
void Game::SetupConfigs()
{
	...
	c_WristHUDRadarHeightStretch = config.RegisterFloat("WristHUDRadarHeightStretch", "Stretches the radar element taller (>1) or shorter (<1) independently of its width", 0.77f);
	c_WristHUDOffset = config.RegisterVector3("WristHUDOffset", "The (forward, left, up) offset of the wrist HUD relative to the off hand controller", Vector3(-0.154457f, 0.11f, 0.0374136f));
	...
	c_WristHUDRadarOffset = config.RegisterVector3("WristHUDRadarOffset", "Fine (forward, left, up) position of just the radar element, on top of the shared WristHUDOffset", Vector3(-0.00615853f, 0.0f, -0.00788247f));
	...
	c_WristHUDRadarRotation = config.RegisterVector3("WristHUDRadarRotation", "Extra rotation in degrees (roll, pitch, yaw) for just the radar element, on top of the shared WristHUDRotation", Vector3(0.0f, 10.0f, 1.0f));
	...
	c_WristHUDAmmoVMin = config.RegisterFloat("WristHUDAmmoVMin", "Top edge (0-1) of the ammo crop from the wrist HUD texture", 0.02f);
	c_WristHUDAmmoUMax = config.RegisterFloat("WristHUDAmmoUMax", "Right edge (0-1) of the ammo crop from the wrist HUD texture", 0.42f);
	c_WristHUDAmmoVMax = config.RegisterFloat("WristHUDAmmoVMax", "Bottom edge (0-1) of the ammo crop from the wrist HUD texture", 0.3f);
	c_WristHUDHealthUMin = config.RegisterFloat("WristHUDHealthUMin", "Left edge (0-1) of the health crop from the wrist HUD texture", 0.58f);
	c_WristHUDHealthVMin = config.RegisterFloat("WristHUDHealthVMin", "Top edge (0-1) of the health crop from the wrist HUD texture", 0.02f);
	c_WristHUDHealthUMax = config.RegisterFloat("WristHUDHealthUMax", "Right edge (0-1) of the health crop from the wrist HUD texture", 1.0f);
	c_WristHUDHealthVMax = config.RegisterFloat("WristHUDHealthVMax", "Bottom edge (0-1) of the health crop from the wrist HUD texture", 0.3f);
	...
	c_OffHandPoseForOneHanded = config.RegisterBool("OffHandPoseForOneHanded", "Hold the two hand grip while carrying a one handed weapon (pistol, plasma pistol, plasma rifle, needler) to snap the off hand to a supporting pose on the weapon. Purely visual - aiming is unchanged. Weapons with no pose recorded in VR/poses/offhandposes.txt keep following the controller", true);
	...
	c_TwoHandPitchOffsetShotgun = config.RegisterFloat("TwoHandPitchOffsetShotgun", "Pitch correction in degrees applied to two-handed aim for the Shotgun, so gripping does not shift aim away from where the weapon was already pointing. Measured value", -3.96f);
	c_TwoHandYawOffsetShotgun = config.RegisterFloat("TwoHandYawOffsetShotgun", "Yaw correction in degrees applied to two-handed aim for the Shotgun, alongside TwoHandPitchOffsetShotgun. Automatically mirrored in left handed mode", 2.22f);
	c_TwoHandPitchOffsetSniper = config.RegisterFloat("TwoHandPitchOffsetSniper", "Pitch correction in degrees applied to two-handed aim for the Sniper, so gripping does not shift aim away from where the weapon was already pointing. Measured value", -4.62f);
	c_TwoHandYawOffsetSniper = config.RegisterFloat("TwoHandYawOffsetSniper", "Yaw correction in degrees applied to two-handed aim for the Sniper, alongside TwoHandPitchOffsetSniper. Automatically mirrored in left handed mode", -3.72f);
	c_TwoHandPitchOffsetRocket = config.RegisterFloat("TwoHandPitchOffsetRocket", "Pitch correction in degrees applied to two-handed aim for the RocketLauncher, so gripping does not shift aim away from where the weapon was already pointing. Measured value", 4.05f);
	...
	c_LeftHandFlashlightDistance = config.RegisterFloat("LeftHandFlashlight", "Bringing the left hand within this distance of the head turns the flashlight on. Higher = works from further away, lower = must bring your hand in closer. <0 to disable", 0.2f);
	c_RightHandFlashlightDistance = config.RegisterFloat("RightHandFlashlight", "Bringing the right hand within this distance of the head turns the flashlight on. Higher = works from further away, lower = must bring your hand in closer. <0 to disable", 0.2f);
	c_HUDToggleDistance = config.RegisterFloat("HUDToggleDistance", "How close the hand opposite the flashlight hand must get to the HUD toggle zone to show/hide the floating HUD. Higher = triggers from further away, so the gesture is easier to reach but more likely to fire by accident. Lower = must bring your hand in closer, less likely to trigger by accident. <0 disables the gesture entirely. Kept smaller than the flashlight distance on purpose: the hand must also leave a zone 2.2x this size before toggling again, so a larger starting value leaves less real-world gap and can toggle repeatedly", 0.15f);
	c_HUDToggleOffset = config.RegisterVector3("HUDToggleOffset", "Where the HUD toggle zone sits relative to your headset: (forward, left, up) in metres. Mirrored horizontally so it sits beside whichever hand is actually used for the toggle. Move this to change WHERE you reach to trigger the gesture; use HUDToggleDistance to change how close you need to get once you are reaching there", Vector3(-0.05f, -0.18f, 0.0f));
	...
	c_LeftShoulderHolsterActivationDistance = config.RegisterFloat("LeftShoulderHolsterDistance", "How close your dominant hand must reach to the left shoulder holster to change weapons - effectively the holster's size. Higher = easier to reach, but more likely to trigger by accident. Lower = must reach closer, less likely to trigger by accident. <0 to disable", 0.3f);
	...
	c_RightShoulderHolsterActivationDistance = config.RegisterFloat("RightShoulderHolsterDistance", "How close your dominant hand must reach to the right shoulder holster to change weapons - effectively the holster's size. Higher = easier to reach, but more likely to trigger by accident. Lower = must reach closer, less likely to trigger by accident. <0 to disable", 0.3f);
